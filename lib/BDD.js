...
var BDD = (() => {
    ...
    BDD.prototype.size = function (seen) {
        let res = 1;
        if (seen === undefined) {
            seen = new Set();
        }
        if (!seen.has(this.onTrue)) {
            seen.add(this.onTrue);
            res += this.onTrue.size(seen);
        }
        if (!seen.has(this.onFalse)) {
            seen.add(this.onFalse);
            res += this.onFalse.size(seen);
        }
        return res;
    };
