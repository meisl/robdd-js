...
//        console.log(" --> " + result.toIteStr());
    ...
    function BDD(l, t, e, neg) {
        ...
        instCount++;
        if (neg === undefined) {
            // NOTE: *must* calculate height before creating neg since in neg's creation we'll use *this*' height
            Object.defineProperty(res, "height", { value: Math.max(t.height, e.height) + 1  });
            ...
            Object.defineProperty(res, "height", { value: neg.height    });
    ...
    function toSrc(bdd) {
        return isTerminal(bdd)
            ? (bdd === True ? "T" : "F")
            : "g('" + bdd.label + "', " + toSrc(bdd.onTrue) + ", " + toSrc(bdd.onFalse) + ")"
        ;
    };
    BDD.prototype.toSrc = function () {
        let result = "p => { let T = p.True, F = p.False, g = p.get; ";
        return result + "return " + toSrc(this) + " }"
    };
    ...
    return {
        ...
        isBDD:      p => p instanceof BDD,

        var:        label => get(label, True, False),
        get:        (l, t, e) => ite(get(l, True, False), t, e),
