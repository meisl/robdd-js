...
//        console.log("and_arr([" + cs.map(x => x.toIteStr()).join(', ') + "])");
        let n = cs.length,
            ...
            lo = -1,
            i  = 0;
        while (i < n) { // to start with, find first non-terminal conjunct
            min = cs[i++];
            if (min === False) {
//                console.log(" --> F");
                return False;
            } else if (min !== True) {  // min is the first non-terminal conjunct, i points after it in cs
                lo = n - i; // lo is now the nr of remaining conjuncts (excl. min itself)
                break;
            }
        }
        if (lo === -1) {  // all cs were True
//            console.log(" --> T");
            return True;
...
//            console.log(" --> " + min.toIteStr());
            ...
            */
            if (k === hi) { // both have the same label
...
//        console.log(" --> " + result.toIteStr());
    ...
    const and2 = function (p, q) {
        andCalls++;
        if ((q === True) || (q === p)) {
            return p;
        } else if ((q === False) || (p === False) || (q === p.not())) {
            return False;
        } else if (p === True) {
            return q;
        } else {
    ...
    const not  = (p)    => p.not();
    const or   = (p, q) => p.or( q);
    const eqv  = (p, q) => p.eqv(q);
    const xor  = (p, q) => p.xor(q);
    ...
    BDD.prototype.not        = function ()      { notCalls++; return this.neg       };
    BDD.prototype.and        = function (p)     { return and2(this, p)              };
    BDD.prototype.or         = function (p)     { orCalls++;  return this.ite(True, p)          };
    BDD.prototype.eqv        = function (p)     { eqvCalls++; return this.ite(p, p.not())       };
    BDD.prototype.xor        = function (p)     { xorCalls++; return this.ite(p.not(), p)       };
