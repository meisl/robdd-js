...
Object.defineProperties(SlotAllocator.prototype, {
    ...
    insist_inUse:   {
        value: function (s, i) {
            if ((s >= 0) && (s < this.maxLen)) {
                let k = this._usable.indexOf(s);
                if ((k < 0) || (k >= this._u)) { // ensure it's not contained in this._usable
                    return s;
                }
            }
            throw new Error("invalid arg" + i + " = " + s + ": currently not used / maxLen: " + this.maxLen + " / usable: " + JSON.stringify(this._usable.slice(0, this._u)));
        }
    },
    get: { value: function () {
        ...
        while (i < n) {
            let a = this.insist_inUse(as[i], i++); // check validity of a, and keep it
