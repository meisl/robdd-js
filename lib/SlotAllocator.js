...
function recycle(self, slot) {
    self._usable[self._u++] = slot; // TODO: use a Set only - its iterator supports concurrent modification!
    self._usableSet.add(slot);
    return self;
}
...
function popUsable(self) {
    ...
    self.popCount++;
    let slot = self._usable[--self._u]; // TODO: use a Set only - its iterator supports concurrent modification!
    self._usableSet.delete(slot);
    return slot;
}
