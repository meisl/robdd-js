...
Program.prototype.traverse = function (chooseTarget, onAfter, onEnd) {
    ...
    this.run((targetSlot, label, thenSlot, elseSlot) => {
        ...
        newTargetSlot = chooseTarget(maxLen, info, Object.keys(entry).map(x => +x));
...
/*
        if (usable[i + 1] !== undefined) {
            Object.keys(usable[i + 1]).forEach(slot => {
                slot = +slot;
                if (!((slot >= 0) && (slot <= maxLen))) {
                    throw new Error("newTargetSlot=" + newTargetSlot + ", maxLen=" + maxLen + ", i=" + i + util.inspect(usable));
                }
            });
        }
*/
