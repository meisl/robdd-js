...
Object.defineProperty(Program.prototype, "labelDeltas", {
    get: function () {
            let result = [],
                i = 0;
            this.run(() => { result.push(this.info.get(i++).labelDelta) });
            return result;
        }
});
...
Program.prototype.run = function (cb) {
    let self      = this,
        labelIdxs = self.init();
    labelIdxs[0] = self.labelIdx(labelIdxs[0]);
    labelIdxs[1] = self.labelIdx(labelIdxs[1]);
    for (let i = 0, n = self.instructionCount; i < n; i++) {
        this.decode(i, function (targetSlot, _label, thenSlot, elseSlot, _labelIdx) {
            let info       = self.info.get(i),
                labelDelta = info.labelDelta,
                labelIdx   = labelDelta + Math.max(labelIdxs[thenSlot], labelIdxs[elseSlot]);
            assert.same(labelIdx, _labelIdx, "labelIdx at " + i);
...
function addInfo(info, bdd, slot) {
    ...
    if (bdd.isTerminal) {
        ...
        let i  = prg.instructionCount,
            t  = bdd.onTrue,
            e  = bdd.onFalse,
            tl = t.isTerminal ? -1 : prg.labelIdx(t),
            el = e.isTerminal ? -1 : prg.labelIdx(e),
            m  = Math.max(tl, el),
