...
function Program(o, opts) {
    ...
    if (BDD.isBDD(o)) {
        ...
        Object.defineProperty(this, "srcOpsRLE",        { value: RLE.init(),    enumerable: true });
        if (o.isTerminal) {
        ...
        Object.defineProperty(this, "srcOpsRLE",      { value: RLE.fromJSON(o.srcOpsRLE),      enumerable: true });
...
Object.defineProperty(Program.prototype, "labelDeltas", {
    value: function () { return this.labelDeltasRLE.values(); },
    enumerable: true,
});

Object.defineProperty(Program.prototype, "srcOps", {
    value: function () { return this.srcOpsRLE.values(); },
    enumerable: true,
});
...
Program.prototype.toJSON = function () {
    ...
    */
    ...
    let targetSlots = RLE.init(),
        thenSlots   = RLE.init(),
        elseSlots   = RLE.init(),
        n = this.instructionCount;
    for (let i = 0; i < n; i++) {
        let info = this.getInfo(i);
        targetSlots.add(info.slot);
        thenSlots.add(info.thenSlot);
        elseSlots.add(info.elseSlot);
    }
    ...
    return {
        ...
        RLE_length:     [this.labelDeltasRLE, this.srcOpsRLE, targetSlots, thenSlots, elseSlots].reduce((acc, rle) => acc + rle.encodedLength, 0),
        labels:         this.labels,
        //labelDeltas: [...this.labelDeltas()],
        labelDeltasRLE: this.labelDeltasRLE,
        srcOpsRLE:      this.srcOpsRLE,
        targetSlotsRLE: targetSlots,
        thenSlotsRLE:   thenSlots,
        elseSlotsRLE:   elseSlots,
    };
};
...
function addInfo(info, bdd, p, q) {
    ...
    if (bdd.isTerminal) {
        ...
        let srcOp = 0;
        if (thenChild.isSwapped) srcOp |= FLAG_SWAPTHEN;
        if (thenChild.isFlipped) srcOp |= FLAG_FLIPTHEN;
        if (elseChild.isSwapped) srcOp |= FLAG_SWAPELSE;
        if (elseChild.isFlipped) srcOp |= FLAG_FLIPELSE;
        prg.srcOpsRLE.add(srcOp);

        thenChild.parents.push(entry);
        //assert.same(Math.max(...thenChild.parents.map(p => p.instruction)), entry.instruction,
        //    "thenChild.maxParent from .parents: [" + thenChild.parents.map(p => p.instruction).join(', ') + "]");
        Object.defineProperty(entry, "thenChild", { value: thenChild, enumerable: true, configurable: false });
        thenChild.maxParent = entry.instruction;
        thenChild.minParent = Math.min(thenChild.minParent, entry.instruction);

        elseChild.parents.push(entry);
        Object.defineProperty(entry, "elseChild", { value: elseChild, enumerable: true, configurable: false });
        //assert.same(Math.max(...elseChild.parents.map(p => p.instruction)), entry.instruction,
        //    "elseChild.maxParent from .parents: [" + elseChild.parents.map(p => p.instruction).join(', ') + "]");
        elseChild.maxParent = entry.instruction;
        elseChild.minParent = Math.min(elseChild.minParent, entry.instruction);
