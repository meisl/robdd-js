...
Program.prototype.toJSON = function () {
    ...
    */
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (info.swapThen) x += 1;
        if (info.flipThen) x += 2;
        if (info.swapElse) x += 4;
        if (info.flipElse) x += 8;
        srcOps.add(x);
    }
    ...
    return {
        ...
        RLE_length:     [labelDeltas, targetSlots, thenSlots, elseSlots, srcOps].reduce((acc, rle) => acc + rle.encodedLength, 0),
        labels:         this.labels,
        //labelDeltas: [...this.labelDeltas()],
        labelDeltasRLE: labelDeltas,
