...
Program.prototype.run = function (cb) {
    ...
    for (let i = 0; i < n; i++) {
        this.decode(i, function (targetSlot, _label, thenSlot, elseSlot, _labelIdx) {
            ...
            results  [targetSlot] = cb(label, results[thenSlot], results[elseSlot]);
...
function _serialize(bdd, prg) {
    ...
    if (!(entry = prg.info.get(bdd))) {
        ...
        let swapped = prg.info.get(BDD.get(bdd.label, bdd.onFalse, bdd.onTrue));
        ...
        if (swapped) {
            console.log(">>>>>>>>>>>>>>>>>>>>swap " + (swapCount++) + ":"
                + " swap(" + swapped.slot + ") = "
                + (prg.maxLen-1) + " <- " + thenInfo.slot + ", " + elseInfo.slot
                + "  ~>  " + bdd.toIteStr()
            );
            console.log("replace " + util.inspect(entry) + " | " + entry.code);
            console.log("....by: " + util.inspect(swapped.swapped) + " | " + swapped.swapped.code);
            //return entry;
        }
    }
    return entry;
...
function addParent(parentEntry, isThenChild) {
    ...
    Object.defineProperty(
        ...
        { value: this.slot, enumerable: true, configurable: false }
    );
    Object.defineProperty(
        parentEntry,
        isThenChild ? "swapThen" : "swapElse",
        { value: this.isSwapped, enumerable: true, configurable: false }
    );
}
...
Object.defineProperties(InfoEntry.prototype, {
            ...
            if (this._swapped === undefined) {
                Object.defineProperty(this, "_swapped", {
                    value: Object.create(this, {
                        isSwapped: { value: true, enumerable: true },
                        _swapped:  { value: this, enumerable: true },
                    }),
                    enumerable: true,
                });
    ...
    code: {
        get: function () {
            let n      = (this.program.instructionCount + "").length,
                result = [" ".repeat(n), ":", this.slot, "<-"];
