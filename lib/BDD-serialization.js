...
Program.prototype.traverse = function (chooseTarget, onAfter, onEnd) {
    ...
    Object.defineProperties(reusableNow, {
        add: { value: function (xs) { if (xs !== undefined) { Object.assign(this, xs) } }, enumerable: false, writable: false },
    ...
    this.run((targetSlot, label, thenSlot, elseSlot) => {
        ...
        reusableNow.add(reusable[i]);

        newTargetSlot = chooseTarget(maxLen, info, Object.keys(reusableNow).map(x => +x), Object.keys(reusable[i+1] || {}).map(x => +x));
...
*/
        ...
        delete reusableNow[newTargetSlot];  // remove newTargetSlot from reusableNow:
