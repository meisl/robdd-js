...
Program.prototype.optimize = function () {
    let usable = new Array(this.maxLen),
        ...
        lsSlots = this.elseSlotsRLE.clear();
    this.traverse(
        /* onInstr: */ (maxLen, instr, newTargetSlot, reusableNow) => {
            let i = instr.instruction,
                thenSlot = instr.thenSlot, reuseThen = instr.reuseThen,
                elseSlot = instr.elseSlot, reuseElse = instr.reuseElse,
                poppedUsable = false;
            if (reuseThen) {
            ...
            thSlots.add((thenSlot << 3) | (reuseThen ? FLAG_REUSE : 0) | (instr.swapThen ? FLAG_SWAP : 0) | (instr.flipThen ? FLAG_FLIP : 0) );
            lsSlots.add((elseSlot << 3) | (reuseElse ? FLAG_REUSE : 0) | (instr.swapElse ? FLAG_SWAP : 0) | (instr.flipElse ? FLAG_FLIP : 0) );

            let assert_same = (actual, expected, what) => {
                if (actual !== expected) {
                    let msg = what + " at instruction #" + i + ":\n";
                    msg += "usable=" + JSON.stringify(usable.slice(0, u)) + "\n";
                    msg += "reusableNow=" + JSON.stringify(reusableNow) + "\n";
