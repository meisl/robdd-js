...
Program.prototype.optimize = function () {
    ...
    this.run((targetSlot, label, thenSlot, elseSlot) => {
        ...
        if (newTargetSlot !== targetSlot) {
            info.slot = newTargetSlot; // change target slot;
            // fix subsequent occurrences
            for (let j of info.readAt) {
                this.instructions[j] = newTargetSlot;   // XXX
            };
        }
        i++;
    });
...
function _serialize(b, p, labels2Idx) {
    ...
    if (entry === undefined) {
        ...
        thenInfo.parents.push(i);
        thenInfo.readAt .push(i * 4 + 2);    // XXX
        elseInfo.parents.push(i);
        elseInfo.readAt .push(i * 4 + 3);    // XXX
...
Object.defineProperties(InfoEntry.prototype, {
    maxParent: { get: function () { return Math.max.apply(null, this.parents) }, enumerable: true },
    minParent: { get: function () { return Math.min.apply(null, this.parents) }, enumerable: true },
    slot:      { get: function () { return this.program.instructions[this.instruction * 4] },     // XXX
                 set: function (s) {
                     if (s !== this.slot) {
                         this.program.instructions[this.instruction * 4] = s;    // XXX
                         // TODO: fix subsequent uses
                     }
                     return s;
                 },
                 enumerable: true,
...
function addInfo(info, bdd, slot, instruction) {
    let program = info.program,
        entry = Object.create(InfoEntry.prototype, {
            program:     { value: program,     enumerable: true,  writable: false },
