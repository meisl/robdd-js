...
Program.prototype.traverse = function (chooseTarget, onAfter, onEnd) {
    ...
    this.run((targetSlot, label, thenSlot, elseSlot) => {
        ...
        recycle(maxLen, i); // add maxLen as possible choice for newTargetSlot
        let entry = reusable[i];

        newTargetSlot = chooseTarget(maxLen, info, Object.keys(entry).map(x => +x), Object.keys(reusable[i+1] || {}).map(x => +x));
        if (!(Number.isSafeInteger(newTargetSlot) && (entry[newTargetSlot] === true))) {
            throw new TypeError("invalid newTargetSlot returned: " + util.inspect(newTargetSlot) + " / choices: [" + Object.keys(entry).join(', ') + "]");
        }
        maxLen = Math.max(newTargetSlot + 1, maxLen);
        // remove newTargetSlot from entry:
        delete entry[newTargetSlot];
        // now pull over the remaining usable slots to next iteration
        let next = reusable[i + 1];
        if (next === undefined) {
            reusable[i + 1] = entry;
        } else {
            ...
            reusable[i + 1] = entry;
            /*
            // push contents from smaller onto larger
...
*/
        delete reusable[i]; // don't need this anymore
...
Program.prototype.toString = function () {
    ...
    this.traverse(
        ...
        /* onAfter: */ (maxLen, info, newTargetSlot) => {
            ...
            result.push(
                util.format("/* %s %s*/ g[%s](%s, %s, %s); // minP: %s, maxP: %s, maxLen: %s, usable: [%s], nextUsable: [%s], subst: %s",
                    mark,
